...
contract Examiner is Initializable{
    ...
    constructor(address factory_, address forwarder_, address org_) {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
    ...
    // restricted
    function changeReceiver(address newReceiver) external controlAccess notClosed{
        receiver = newReceiver;
    }

    function changeAdminStatus(address admin_) external controlAccess notClosed{
        bool prevState = admin[admin_];
        admin[admin_] = !prevState;
    }

    function allowToken(address token) external controlAccess notClosed{
        allowedTokens[token] = true;
    }

    function close() external controlAccess(){
        closed = true;
    }
