 ...
 */
contract SzeetaAdministratorV3 is EIP712{
    /**
    ...
    * where the event id is mapped to a root hash of a merkle tree generated by chainData
    * as a proof to validate cached data off-chain.
    */
    mapping(uint256 => bytes32) public receivers;
     ...
     */
    function createEvent(address owner, bytes32 dataHash, ChainData[] memory chainData) external onlyOrg returns(uint){
        /**
         * @dev Assigning event id to a local variable to save gas
         */
        uint eventId = eventCounter;
        owners[eventId] = owner;
        receivers[eventId] = dataHash;
        ...
        onlyOrg
    {
        ...
        receivers[eventId] = receiverList;

        emit ReceiverChanged(newReceiver, eventId);
    }
